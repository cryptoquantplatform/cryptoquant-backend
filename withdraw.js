...
async function loadUserBalance() {
    try {
        ...
        if (data.success) {
            const availableBalance = parseFloat(data.availableBalance || data.totalEarnings || 0);
            ...
            if (balanceElement) {
                balanceElement.textContent = availableBalance.toFixed(2) + ' USD (Earnings Only)';
            }
        }
    } catch (error) {
        ...
        const userData = JSON.parse(localStorage.getItem('userData'));
        const availableBalance = userData ? (userData.availableBalance || 0) : 0;
