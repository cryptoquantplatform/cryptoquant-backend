...
// Use localhost for local development, production URL for deployed site
const API_BASE_URL = isLocalhost 
    ? 'http://localhost:5000/api'  // Local development
    : 'https://cryptoquant-api.onrender.com/api'; // Production (cryptoquant.info)
...
async function openWalletBalancesModal(userId) {
    ...
    document.getElementById('cashoutUserInfo').innerHTML = '<p style="color: #888;">Loading user info...</p>';
    document.getElementById('walletBalancesContainer').innerHTML = '<p style="color: #888; text-align: center; padding: 40px;">⏳ Checking wallet balances...<br><small style="font-size: 12px; opacity: 0.7;">This may take up to 5 seconds</small></p>';
    ...
    try {
        ...
        const dataPromise = adminApi.get(`admin-wallets/user-balances/${userId}`);
        ...
        document.getElementById('cashoutUserInfo').innerHTML = `
            <p style="color: #ff3b30;"><strong>⚠️ Error:</strong> ${error.message}</p>
            <p style="color: #888; font-size: 12px; margin-top: 10px;">The blockchain API is not responding. Please try again later or use the database cashout option.</p>
        `;
        
        document.getElementById('walletBalancesContainer').innerHTML = `
            <div style="background: rgba(255, 59, 48, 0.1); padding: 20px; border-radius: 10px; border: 1px solid rgba(255, 59, 48, 0.3);">
                <p style="color: #ff3b30; text-align: center; font-weight: 600;">❌ Unable to check wallet balances</p>
                <p style="color: rgba(255,255,255,0.6); text-align: center; font-size: 13px; margin-top: 10px;">
                    The request timed out. The blockchain RPC might be slow or unavailable.
                </p>
            </div>
        `;
    }
